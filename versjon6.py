...
pygame.init()
pygame.display.set_caption("Silent Ride")
...
def play():
    ...
    text = font.render("W:Akselerer S:Retardere SPACE:Startfart 1:Turbo", True, (76, 0, 153))
    text_rect = text.get_rect(center=(bredde//4.5 + 40, høyde//4.5 - 180))
    ...
    while True:
        ...
        if keys[pygame.K_a] or keys[pygame.K_LEFT]:
            ...
            if bil_x < 0:  # Check if car is going beyond the left boundary
                bil_x = 0  # Reset car position to left boundary
        if keys[pygame.K_d] or keys[pygame.K_RIGHT]:
            bil_x += 5  # Adjust car position to the right
            if bil_x > bredde - bil.get_width():  # Check if car is going beyond the right boundary
                bil_x = bredde - bil.get_width()  # Reset car position to right boundary
        if keys[pygame.K_w] or keys[pygame.K_UP]:
            hastighet = 20  # Accelerate the car
        if keys[pygame.K_s] or keys[pygame.K_DOWN]:
            hastighet = 5  # Decelerate the car
        if keys[pygame.K_SPACE]:
            hastighet = 10  # Set the car speed to default
        if keys[pygame.K_1]:
            hastighet = 40  # Activate turbo mode
